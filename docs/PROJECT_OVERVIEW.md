...
- **Меню**: `0/00/000` или кнопка → «Профиль/Продажа/Покупка».
- **Продажа**:
...
- Маршрутизатор (`runner.py`) вешает хендлеры на incoming/outgoing события. Основные текстовые триггеры: `/start`, `баланс`, `0/00/000`, `профиль/продажа/покупка`.
...
4. Проверка: отправить `0` → меню, «Покупка» → «Все объявления», затем цифру.
