...
- В меню «Продажа» доступны подпункты:
  ...
  - «Мои объявления» показывает статистику по объявлениям в БД (кол-во, активные, несколько последних записей). После списка можно написать `объявление 1` или `объявление ID42`, чтобы увидеть конкретную карточку и фото.
- В меню «Покупка» сейчас доступно 3 плейсхолдера (кнопки «Все объявления», «Фильтр поиска», «Избранное»). Они отправляют текстовые stub-ответы и помогут нарастить функциональность позже. Для быстрого теста можно писать эти же фразы текстом.
- Кнопка «Все объявления» в разделе покупки выводит подборку последних активных объявлений из базы (ID, цена, пробег). Фильтры и избранное пока заглушки.
